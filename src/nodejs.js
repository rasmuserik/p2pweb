...
const port = process.env.P2PWEB_PORT || 3535;
const url = process.env.P2PWEB_URL || 'ws://localhost:3535'; // TODO no default
...
wss.on('connection', function connection(ws) {
  ...
  ws.on(
    'message',
    msg =>
      con.onmessage && con.onmessage({data: JSON.parse(msg), con: con})
  );
...
networkAbstraction.receiveSignalling = o => {
  o.onmessage = msg => {
    ...
    if (msg.websocket) {
      ...
      ws.on('error', () => {
        console.log('could not connect to ' + msg.websocket);
      });
    } else {
